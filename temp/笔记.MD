...
update(Wrappers.<EZhangTongAlarmConfigPO>lambdaUpdate()
       .set(EZhangTongAlarmConfigPO::getStatus, false)
       .eq(EZhangTongAlarmConfigPO::getIndicator, indicator)
       .eq(-1 != cityOid, EZhangTongAlarmConfigPO::getCityOid, cityOid)
       .eq(EZhangTongAlarmConfigPO::getEnv, env));
```
...
```java
@Data
public class ElementIndexVO {

    private String en;

    private List<Float> successNumList;

    private List<Float> totalValueList;
}
```
...
```xml
<resultMap id="elementIndexMap" type="com.example.ElementIndexVO">
    <result column="element_name" jdbcType="VARCHAR" property="en"/>
    <collection property="successNumList" javaType="ArrayList" ofType="java.lang.Float">
        <constructor>
            <arg column="success_num"/>
        </constructor>
    </collection>
    <collection property="totalValueList" javaType="ArrayList" ofType="java.lang.Float">
        <constructor>
            <arg column="total_value"/>
        </constructor>
    </collection>
</resultMap>

<select id="listCurrntElementIndex" resultMap="elementIndexMap">
    SELECT
        element_name,
        success_num,
        total_value
...
```java
@MapKey("en")
Map<String, ElementIndexVO> listCurrntElementIndex();
```
...
```
String jsonString = "{\"data\":[{\"id\":1,\"name\":\"Item1\"},{\"id\":2,\"name\":\"Item2\"}]}";
```
...
Response<Item> response = new ObjectMapper().readValue(jsonString, new TypeReference<Response<Item>>() {})

List<Item> list = new ObjectMapper().readValue(jsonString, new TypeReference<List<Item>>() {})
```
