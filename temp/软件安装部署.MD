...
File -> Settings -> Build，Excution，Deployment -> Build Tools -> Maven 后

设置 Maven home directory

设置 User setting files，选中Override 选择 maven home 目录下的 conf/settings.xml

设置 Local Repository，为本地 maven 仓库目录
...
打开 idea -> Setting -> Version Control -> 点击右上角的 + 号 添加 github 的账号密码 -> Log In



### 从 github 上下载项目

打开 idea -> Get from Version Control 

URL 填写 github 上项目的 Clone 链接，Directory 填写项目目录
...
从官网 http://www.jetbrains.com/idea/download/#section=windows 下载解压版的 idea 并解压即可

### 激活

https://www.cnblogs.com/linliquan/p/10631768.html

把 jar 包 JetbrainsCrack.jar，把它放到你认为合适的文件夹内

打开 idea 在 Configure -> Edit Custom VM Options 中添加如下：
...
打开 idea 先试用30天，然后在 Configure -> License Manage中

选择 License server 方式，地址填入：http://jetbrains-license-server



### 汉化

去 github 搜索 [jetbrains-in-chinese](https://github.com/pingfangx/jetbrains-in-chinese) 点击进入，找到IntelliJ IDEA的目录，点击进入目录界面，然后点击进入想要汉化的对应版本，点击 Download 下载汉化文件

关闭软件。把下载的汉化文件复制粘贴到IntelliJ IDEA的安装目录下的lib子目录下

再次打开软件，我们就会看到软件已经显示为简体中文了。

至此，IntelliJ IDEA汉化为简体中文成功

### 配置 jdk

关闭项目后点击 Confogure -> Structure for New Projects -> SDKs

点击 + 号选择 jdk 安装目录即可

然后点击 Project 选择你的默认 jdk
...
左边切到Includes 在File Header编辑框中，编写如下代码
...
Settings -> Editor -> Live Templates -> 点击右边+号 -> Template Group 

输入CommentGroup

然后选择CommentGroup -> 点击右边+号 -> Live Template

Abbreviation 输入*, Expand with选择Tab
