...
1.初始化 nacos 相关表：运行 **conf/nacos-mysql.sql** 文件
...
`blockHandler / blockHandlerClass`：blockHandler 对应处理 BlockException 的函数名称，可选项。blockHandler 函数访问范围需要是 public，返回类型需要与原方法相匹配，参数类型需要和原方法相匹配并且最后加一个额外的参数，类型为 BlockException 。blockHandler 函数默认需要和原方法在同一个类中。若希望使用其他类的函数，则可以指定 blockHandlerClass 为对应的类的 Class 对象，注意对应的函故必需为 static 函数，否则无法解析。
...
`defaultFallback` (since 1.6.0)：默认的 fallback 数名称，可选项，通常用于通用的 fallback 逻辑（即可以用于很多服务或方法）。默认 fallback 函教可以针对所有类型的异常(除了 `exceptionsToIgnore` 里面排除掉的异常类型）进行处理。若同时配置了 `fallback` 和`defaultFallback`，则只有 `fallback` 会生效。`defaultFallback` 函数签名要求：

- 返回值类型必须与原函数返回值类型一致。
- 方法参数列表需要为空，或者可以额外多一个 Throwable 类型的参数用于接收对应的异常。
- `defaultFalback` 函数默认需要和原方法在同一个类中。若希望使用其他类的图数，则可以指定 `fallbackClass` 为对应的类的 Class 对象，注意对应的函数必需为 static 函数，否则无法解析。
...
[(13条消息) 【Sentinel 规则持久化配置】_sentinel配置持久化_BridgeCloud的博客-CSDN博客](https://blog.csdn.net/qq_37168484/article/details/130550929)

[(13条消息) Sentinel规则持久化（1.8.+版）_sentinel持久化规则官方文档_陌守的博客-CSDN博客](https://blog.csdn.net/qq_36763419/article/details/121560105)
...
cd nginx-1.24.0
# sudo apt install openssl libssl zlib1g-dev
# --with-stream : 支持 TCP 转发
sudo ./configure --prefix=/opt/nginx --with-stream --with-http_stub_status_module --with-http_ssl_module --with-pcre=/opt/pcre-8.45
sudo make && make install
/opt/nginx/sbin/nginx -v
#添加用户组(-r 创建系统工作组,系统工作组的组 ID 小于 500)
sudo groupadd -r www
#添加用户(-r 建立系统帐号,-s /bin/false 参数指定 www 用户仅拥有所有权,而没有登录权限)
sudo useradd -r -g www -s /bin/false www
sudo chown -R www:www /opt/nginx
...
```shell
sudo /opt/nginx/sbin/nginx					    # 启动 Nginx
sudo /opt/nginx/sbin/nginx -s reload            # 重新载入配置文件
sudo /opt/nginx/sbin/nginx -s reopen            # 重启 Nginx
sudo /opt/nginx/sbin/nginx -s stop              # 停止 Nginx
sudo /opt/nginx/sbin/nginx -V					# Nginx 有哪些模块
