...
public static void main(String[] args) {
    ...
    // nacos
    // $2a$10$OUohm24WhHoYRC4fADWKXOdXptJTKNN04RMyvhW86hGwxbUDuE/Mq
}
```
...
sudo -u sentinel nohup java -Dserver.port=8050 -Dcsp.sentinel.dashboard.server=localhost:8050 -Dproject.name=sentinel-dashboard -Dsentinel.dashboard.auth.username=sentinel -Dsentinel.dashboard.auth.password=sentinel -Dcsp.sentinel.log.dir=/var/log/sentinel -Dcsp.sentinel.log.use.pid=false -Dcsp.sentinel.api.port=8719 -jar /opt/apps/sentinel/sentinel-dashboard.jar > /var/log/sentinel/sentinel.nohup.log 2>&1 &
...
#### OpenFeign 整合 Sentinel(SpringCloudAlibaba)

##### pom.xml

参考 Openfeign 和 Sentinel 的配置
...
```properties
# txServiceGroup=spring_cloud_alibaba_tx_group
# service.vgroupMapping.spring_cloud_alibaba_tx_group=default
# 对应 seata server 的 seata.seaver.server-port
# service.default.grouplist=192.168.31.100:8091
# support: nacos, eureka, redis, zk, consul, etcd3, sofa
registry.type=nacos
# registry.nacos.application=seata-tc-server
registry.nacos.serverAddr=192.168.31.100:8848
registry.nacos.namespace=ed2069f3-cc54-4a62-b044-95061643ae84
registry.nacos.group=MY_GROUP
# registry.nacos.cluster=SH
# #if use MSE Nacos with auth, mutex with username/password attribute
# registry.nacos.access-key=""
# registry.nacos.secret-key=""
...
store.db.url=jdbc:mysql://192.168.31.100:3306/spring_cloud_alibaba_seata?useSSL=false&serverTimezone=Hongkong&useUnicode=true&characterEncoding=utf-8&autoReconnect=true
store.db.user=root
...
使用 script/server/db/mysql.sql 初始化数据库 spring_cloud_alibaba_seata
...
```yaml
seata:
  tx-service-group: spring_cloud_alibaba_tx_group
  service:
    vgroup-mapping:
      spring_cloud_alibaba_tx_group: default
    default:
      # 对应 TC 服务的 IP 及 seata.server.server-port 配置的端口
      grouplist: 192.168.31.100:8091
```
...
sudo ./configure --prefix=/opt/nginx --with-stream --with-http_stub_status_module --with-http_ssl_module
...
检查配置文件 `nginx/sbin/nginx -t`

[参考](https://www.cnblogs.com/54chensongxia/p/12938929.html#:~:text=Nginx的主配置文件,间不存在次序关系。)
