...
​		JWT 是一种紧且自自包含的，用于在多方传递 JSON 对象的技术。传递的数据可以使用数字签名增加其安全性。可以使用 HMAC 加密算法或 RSA 公/私加密方式。
...
​		**注意：即使 JWT 有签名加密机制，但是 payload 内容都是明文记，除非记录的是加密，否则不排除泄露隐私最据的可能。不在payload 中记录任何敏感数据。**
...

{
  ...
  "access_token": "o5KRPWUFx34c5M1ODO6oO6VlyIk",
