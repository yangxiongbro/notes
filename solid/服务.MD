...
编写[脚本](../files/scripts/server/zookeeper.server)文件放在 base_dir 目录下 `/opt/apps/zookeeper/zookeeper.server`
...
# 为新建Topic的默认Partition数量，partition数量提升，一定程度上可以提升并发性，数值应该小于等于broker的数量
num.partitions=3
#listeners=PLAINTEXT://192.168.31.101:9092
...
代表完全匹配，搜索之前不会对你搜索的关键词进行分词，对你的关键字去文档分词库中去匹配内容。
...
```json
POST http://192.168.31.90:10519/sms-logs-index/_search/
{
    "from": 0,
    "size": 5,
    "query": {
...
```json
// id 查询
GET http://192.168.31.90:10519/sms-logs-index/_doc/2O9_VpEBCrSApDIskHH3
...
POST http://192.168.31.90:10519/sms-logs-index/_search/
{
    ...
    "query": {
        "ids": {
            "values": ["1u9_VpEBCrSApDIskHH3", "2O9_VpEBCrSApDIskHH3"]
        }
    }
}
```
...
通配查询，和 MySQL 中的 like 是一个套路，可以在查询时，在字符串中指定通配符 `*` 和占位符 `?`
...
ES 对 from + size 是有限制的，from 和 size 二者之和不能超过 1W
...
```json
// 执行 scro11 查询，返回第一页数据，并且将文档 id 信息存放在 ES 上下文中，指定生存时间 1m
POST http://192.168.31.90:10519/sms-logs-index/_search?scroll=1m
{
    "from": 0,
    "size": 5,
    "query": {
...
// 根据 scro1l 查询下一页数据
POST http://192.168.31.90:10519/_search/scroll
{
    "scroll_id": "FGluY2x1ZGVfY29udGV4dF91dWlkDXF1ZXJ5QW5kRmV0Y2gBFmZlZ19hanZmUm9DVzVWaV94Q3hvTmcAAAAAAAAAFBZNcmV1Wkt0alFIaTM4Si1TZ2FqN013",
    "scroll": "1m"
}

// 删除 scro11 在 ES 上下文中的数据
DELETE http://192.168.31.90:10519/_search/scroll/FGluY2x1ZGVfY29udGV4dF91dWlkDXF1ZXJ5QW5kRmV0Y2gBFmZlZ19hanZmUm9DVzVWaV94Q3hvTmcAAAAAAAAAFBZNcmV1Wkt0alFIaTM4Si1TZ2FqN013
```
...
**如果你需要删除的内容，是 index 下的大部分数据，推荐创建一个全新的 index，将保留的文档内容，添加到全新的索引。**



```JSON
POST http://192.168.31.90:10519/sms-logs-index/_delete_by_query
{
    "query": {
        "range": {
            "fee": {
...
filter，根据你的查询条件去査询文档，不去计算分数，而且 filter 会对经常被过滤的数据进行缓存。
    ...
    SearchResponse<SmsLogsPO> response = client.search(request -> request
                    ...
                    .highlight(h -> h // 高亮查询
                            .fields("smsContent", v -> v
                                    ...
                                    .postTags("</font>"))), // 默认查询 10 条,如果需要查询更多则需要指定 size
            SmsLogsPO.class);
    System.out.println(response.hits().total().value()); //数量
    response.hits().hits().forEach(h -> {
        SmsLogsPO log = h.source(); // 实体类
...
[Aggregations | Elasticsearch Guide ](https://www.elastic.co/guide/en/elasticsearch/reference/8.10/search-aggregations.html)

ES 的聚合查询和 MySQL 的聚合查询类型，ES 的聚合查询相比 MySQL 要强大的多，ES 提供的统计数据的方式多种多样。



```java
// ES 聚合查询的 RESTful 查询语法
POST index/_search
{
    "aggs": {
        "结果属性名": {
...
public void searchDocument() throws IOException {
    SearchResponse<SmsLogsPO> response = client.search(request -> request
                    ...
                    .aggregations("agg_result", a -> a    // 聚合查询
                            .cardinality(c -> c
                                    .field("province"))),
            SmsLogsPO.class);
...
范围统计可以针对普通的数值，针对时间类型，针对 ip 类型都可以做相应的统计。
...
他可以帮你査询指定Field的最大值，最小值，平均值，平方和。。。。
...
ES 中提供了一个数据类型 geo-point，这个类型就是用来存储经纬度的。
...
```json
POST http://192.168.31.90:10519/map-index/_doc/1
{
    "name": "天安门",
    "location": {
        "lon": 116.403981,
...
POST http://192.168.31.90:10519/map-index/_doc/2
{
    "name": "海淀公园",
    "location": {
        "lon": 116.302509,
...
POST http://192.168.31.90:10519/map-index/_doc/3
{
    "name": "北京动物园",
    "location": {
        "lon": 116.34304,
...
```json
POST http://192.168.31.90:10519/map-index/_search
{
    "query": {
        "geo_distance": {
            "location": {				// 确定一个点
