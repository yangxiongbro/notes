...
sudo cat /var/log/soap_auth.log | fgrep 'Jul'
sudo cat /var/log/soap_oper.log | fgrep 'Jul'
sudo wc -l /sidata02/k8slog/188.5.20.59_202304*.log
wc -l /opt/app/garnet/logs/202307/188.5.20.59_202307*.log
cat /opt/app/garnet/logs/202307/188.5.20.59_202307*.log | head -n 10
sudo wc -l /sidata02/k8slog/k8sOperationFor_pov-hansoc.202212*.txt
sudo wc -l /sidata02/k8slog/k8sOperationFor_pov-hansoc.2023*.txt | grep -E '202304|202305|202306'
...
```sql
-- delete from F_HX_APP_LOGS where to_date('2023-04-01 00:00:00','yyyy-mm-dd hh24:mi:ss')<=CREATED_TIME AND CREATED_TIME<=to_date('2023-04-02 23:59:59','yyyy-mm-dd hh24:mi:ss')

select MIN(ID), MAX(ID) from F_HX_APP_LOGS where to_date('2023-04-01 00:00:00','yyyy-mm-dd hh24:mi:ss')<=CREATED_TIME AND CREATED_TIME<=to_date('2023-04-30 23:59:59','yyyy-mm-dd hh24:mi:ss')

select created_time,count(1) from(
	select to_char(CREATED_TIME,'yyyy-mm-dd') as created_time from (
    	SELECT CREATED_TIME FROM F_HX_APP_LOGS WHERE 922469 <= ID AND ID <= 968251
    ) where 
		to_date('2023-07-01 00:00:00','yyyy-mm-dd hh24:mi:ss')<=CREATED_TIME AND CREATED_TIME<=to_date('2023-07-31 23:59:59','yyyy-mm-dd hh24:mi:ss')
)logs
group by created_time
order by created_time asc
```
...
awk '{if(-e cmgde2eas-fe){rm -r cmgde2eas-fe}}' && mv -f cmgde2eas-fe-prodsit cmgde2eas-fe
```
...
188.5.20.59:24599/cmgde2eas-fe/#/analysis-platform/index
