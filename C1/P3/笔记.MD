...
sudo cat /var/log/soap_auth.log | fgrep 'Jun'
sudo cat /var/log/soap_oper.log | fgrep 'Jun'
sudo wc -l /sidata02/k8slog/188.5.20.59_202304*.log
wc -l /opt/app/garnet/logs/202305/188.5.20.59_202304*.log
...
select created_time,count(1) from(
	select to_char(CREATED_TIME,'yyyy-mm-dd') as created_time from (
    	SELECT CREATED_TIME FROM F_HX_APP_LOGS WHERE 765507 <= ID AND ID <= 946579
    ) where 
		to_date('2023-04-01 00:00:00','yyyy-mm-dd hh24:mi:ss')<=CREATED_TIME AND CREATED_TIME<=to_date('2023-04-30 23:59:59','yyyy-mm-dd hh24:mi:ss')
)logs
group by created_time
order by created_time asc
```
